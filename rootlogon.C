...
void rootlogon(TString additional_libs = "",
	...
	gROOT->ProcessLine(".include /scratch/gccb/software/include");
//	gROOT->ProcessLine(".include /opt/sifi-framework/build");
  TString common_libs;

  TString cc6dir = gSystem->GetFromPipe("echo $SIFIDIR");
  Bool_t useSIFIDIR = (cc6dir == "") ? kFALSE : kTRUE;

  TString mycc6dir = gSystem->GetFromPipe("echo $MYSIFIDIR");
  Bool_t useMYSIFIDIR = (mycc6dir == "") ? kFALSE : kTRUE;
...
// common_libs = "CCDetRes Fibers SiFiTools SiFi Geant DesktopDigitizer6";
  common_libs = "Fibers SiFiTools SiFi Geant DesktopDigitizer6";
...
ProcessLibs:
  ...
  full_path = gSystem->Getenv("MYSIFIDIR");
  if (!full_path.IsNull()) {
    full_path += "/include";
  ...
  full_path = gSystem->Getenv("SIFIDIR");
  if (!full_path.IsNull()) {
    full_path += "/include";
    if (!gSystem->AccessPathName(full_path.Data())) {
      full_path.Prepend(".include ");
  ...
  gSystem->SetMakeSharedLib("cd $BuildDir; "
                            "g++ -std=c++17 -c $Opt -pipe -Wall -fPIC -pthread "
