double zestawienie(TString name="single_fibel_hamamatsu_1.root" ){
  
  
  
TFile *f1 = new TFile(name,"READ");
TH1D *h1 = new TH1D("h1", ";tot [ns];Counts", 100, 340, 1100);
...
double cps=t1->GetEntries()/tk;

std::cout<<"cps "<<cps<< std::endl;


tree1->Draw("falling[20]-rising[20]>>h1");
h1->Scale(1./(tk-t0));
h1->Sumw2(kFALSE);
...
gPad->SetRightMargin(0.25);
 ...
 // double errorsigma=fr->GetParError(5);
 // double errormean=fr->GetParError(4); 
 // double error= resolution*sqrt(((errorsigma/sigma)*(errorsigma/sigma))+(errormean/mean)*(errormean/mean));
