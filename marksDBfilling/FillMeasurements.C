...
void FillMeasurements(UInt_t series_id, const char * database_name = "prototype_db.sqlite3") {
    ...
    //only works for series measurements with log files
    const char * query = Form("SELECT series_id, log_file FROM Series WHERE series_id = %d", series_id);
    ...
    for(int i=0; i < results.size(); ++i) {
        ...
        for(UShort_t k = 0; k < tokenlogs.size(); ++k) {            
            ...
            while(logfile.good()){
                ...
                Exec(database, Form("INSERT INTO Measurements (series_id, datadir, start_time, stop_time, duration, source_pos) VALUES (%s, '/scratch/gccb/data/%s/%s', %ld, %ld, %ld, '0;%2.f')", results[i]["series_id"].c_str(), dir, meas_name.Data(), start_time, stop_time, stop_time - start_time, pos_true) );
